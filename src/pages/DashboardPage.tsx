...
export const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  // Stored alert summaries for active alert strip
  const [storedAlerts, setStoredAlerts] = useState(() => getStoredAlerts());

  useEffect(() => {
    setStoredAlerts(getStoredAlerts());
  }, []);

  // Evaluate alerts against live cards
  const evaluatedAlerts = useMemo(() => {
    return storedAlerts
      .map((alt) => {
        const matchingCard = liveCards.find((c) => c.crop === alt.crop);
        const currentModalPrice = matchingCard ? matchingCard.modalPrice : 1850;
        const ev = evaluateAlertStatus(alt);
        return {
          ...alt,
          currentPrice: currentModalPrice,
          distanceToTarget: Math.abs(alt.targetPrice - currentModalPrice),
          evaluatedStatus: ev.status
        };
      })
      .filter((alt) => alt.evaluatedStatus !== 'DISABLED');
  }, [storedAlerts, liveCards]);
